        ...
        <div className="flex gap-3 w-full max-w-2k mx-auto px-5 overflow-x-auto overflow-y-visible lg:px-10 leading-6 lg:text-xl lg:leading-6 font-bold sticky top-0">
          {tabs.map((tab) => {
            ...
            return (
              <Link
                className={cn(
                  'relative px-2 py-3 lg:px-5 whitespace-nowrap text-brand hover:text-brand transition-all border-b-2 border-black',
                  isActive ? 'text-brand border-brand' : 'text-white'
                )}
                href={tab.href}
                key={tab.href}
              >
